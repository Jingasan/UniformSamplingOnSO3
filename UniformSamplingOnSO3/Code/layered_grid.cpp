...
/*** 2(Layered Grid)選択時：指定した解像度レベルまでの点をすべて作成・出力 ***/
bool layered_grid( int resol ){
	...
	for( int l = 0; l <= resol; l++ ){
		...
		/* 回転角Ψの点座標を格納した配列の取得 */
		Psi_Points = grid_s1( l );	// 回転平面 S1 の等間隔サンプリング
		
		/* 回転角Ψの点座標を格納した配列が空のとき */
		if( Psi_Points.size() == 0 ) return false;	// 失敗：0 を返す
		

		/*** 2次元球面 S2 のサンプリング ***/

		/* 2の解像度レベル乗 */
		Nside = pow( 2, (double)l );		// 解像度レベル 0：1, 解像度レベル 1：2, 解像度レベル 2：4, 解像度レベル 3：8, …
