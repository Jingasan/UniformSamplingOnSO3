...
/*** HEALPix：各点における回転角 θ, Φ の算出 ***/
void pix2ang_nest(
	long nside,		// 2の解像度レベル乗
	long ipix,		// 2次元球面 S2 における点座標( θ, Φ )の ID 番号
	double *theta,	// θ
	double *phi		// Φ
){
	...
	*/
	...
	/* 解像度レベルが 0 未満, あるいは 13 超の場合 */
	if( nside < 1 || nside > ns_max ){
		fprintf( stderr, "%s (%d): nside out of range: %ld\n", __FILE__, __LINE__, nside );
		exit( 0 );
	}
	
	/* 2次元球面 S2 における最大点座標数の算出 */
	npix = 12 * nside * nside;
	
	/* 2次元球面 S2 における点座標 ID が 0 未満, あるいは最大点座標数を超える場合 */
	...
	ipf = (int)fmod( (double)ipix, npface );	// ! pixel number in the face {0,npface-1}
	
	//c finds the x,y on the face (starting from the lowest corner)
	//c from the pixel number
	ip_low = (int)fmod( (double)ipf, 1024 );		// ! content of the last 10 bits
	ip_trunc = ipf / 1024;							// ! truncation of the last 10 bits
	ip_med = (int)fmod( (double)ip_trunc, 1024 );	// ! content of the next 10 bits
	...
	z = ( 2 * nside - jr ) * fact2;
	kshift = (int)fmod( (double)( jr - nside ), 2 );
